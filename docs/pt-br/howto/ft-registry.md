...
Registro de número FT → nome do projeto → arquivo howto.
Use para consultar "o que era o FT268?" retroativamente.

> **Dica de geração automática**: Extraído dos cabeçalhos `FT reference` em `docs/howto/`.
> Ao adicionar um novo FT, inclua a linha correspondente neste arquivo.

---

## Registro

| FT | Projeto | Arquivo howto | Tipo |
|----|---------|---------------|------|
...
Ao adicionar um novo FT, inclua a linha abaixo (mantendo a ordem por número de FT):
...
| Tipo | Critério |
|------|---------|
| **ATK** | Inclui teste de ataque de cracker (ATK-01〜12) |
| **VULN** | Inclui diagnóstico de vulnerabilidades (VULN-A〜L / V-01〜10) |
| **Normal** | How-to de implementação sem os anteriores |

### Comando de Geração Automática (Material de Referência)

```bash
# Extrair material de registro dos cabeçalhos FT reference
