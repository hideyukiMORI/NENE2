...
FT 编号 → 项目名称 → howto 文件的目录。
用于查询"FT268 是什么来着？"等问题。

> **自动生成提示**：从 `docs/howto/` 的 `FT reference` 头部提取。
> 添加新 FT 时，需在此文件中追加对应行。
...
| FT311 | expenselog | [expense-tracking-api.md](expense-tracking-api.md) | 通常 |

---

## 维护方法

添加新 FT 时，追加以下行（保持 FT 编号顺序）：
...
| 类型 | 判定 |
|------|------|
| **ATK** | 包含破解者攻击测试（ATK-01 至 ATK-12） |
| **VULN** | 包含漏洞诊断（VULN-A 至 VULN-L / V-01 至 V-10） |
| **通常** | 除上述以外的实现 how-to |

### 自动生成命令（作为素材）

```bash
# 从 FT reference 头部提取目录素材
