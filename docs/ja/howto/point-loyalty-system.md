...
冪等トランザクション（reference_id）・残高下限保護・admin RBAC を解説します。

## 概要

- ユーザーがポイントを獲得（earn）・消費（spend）できます
- admin のみポイントを直接調整（adjust: add/subtract）できます
- 残高はトランザクションの `balance_after` を積み上げて管理します
- `reference_id` による冪等トランザクション（二重付与・二重消費防止）
- `max_uses` ではなく上限金額（MAX_EARN_PER_TRANSACTION）でバルク不正付与を防ぎます
...
`balance_after >= 0` の CHECK 制約が残高マイナスを DB レベルで防ぎます。
`amount > 0` の CHECK 制約が 0 以下のトランザクションを DB レベルで拒否します。
...
最新トランザクションの `balance_after` が現在残高です。
別テーブルに残高を保持しないため、トランザクション履歴が唯一の真実源（SSOT）となります。
...
`reference_id` が既に存在する場合は既存トランザクションを返します（200）。
二重クレジット・二重デビットを防ぎます。
...
アプリ層で残高チェックを行い、DB の CHECK 制約と二重防御します。
...
1 回のトランザクションで大量ポイントを不正付与する攻撃を防ぎます。

## アクセス制御

自分の残高・履歴は本人のみ参照可能です。admin は全ユーザーを参照・操作できます。
...
earn/spend は自分のポイントのみ操作可能です（他人のポイントを自分で増やすことはできません）。
