...
`nene2.dev` は NENE2 フレームワークの公式ドメインです。組み込みのエラー型ページはこのドメインで解決されます。標準のエラーセットに対して設定変更は不要です。

プロジェクト独自の問題型（例: `payment-failed`）を定義し、その URI を自分のドメインで解決させたい場合は、`.env` に `PROBLEM_DETAILS_BASE_URL` を設定してください。
