...
| `…/invalid-json` | 400 | Invalid JSON | リクエストボディが空、構文的に無効、または JSON オブジェクトでない |
| `…/validation-failed` | 422 | Validation Failed | リクエストボディが無効、または必須フィールドが欠落 |
| `…/unauthorized` | 401 | Unauthorized | 保護されたエンドポイントでトークンが欠落または無効 |
| `…/too-many-requests` | 429 | Too Many Requests | レート制限超過（`ThrottleMiddleware`）、`Retry-After` ヘッダーあり |
