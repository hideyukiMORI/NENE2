...
Si votre projet définit des **types de problèmes personnalisés** (ex. `payment-failed`) et que vous souhaitez que ces URIs se résolvent sur votre propre domaine, définissez `PROBLEM_DETAILS_BASE_URL` dans `.env` :
