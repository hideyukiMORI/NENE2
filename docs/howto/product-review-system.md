...
1ユーザー1商品1レビューの制約、評価集計（平均・分布）、CRUD を含む。
...
`UNIQUE(product_id, user_id)` 制約でデータ層から二重レビューを防ぐ。

アプリ層でも事前チェックし、409 Conflict を返す:
...
```php
// is_int() で浮動小数点数を拒否（JSONの 4.5 は float → 422）
...
`product_id` のクロスチェックにより、他商品のレビューIDを指定した場合も 404 を返す。
