...
### IMP-07 `DatabaseConstraintException` を vendor に収録する（v1.5.23 でリリース）
- 発見: 新卒・ロースキル・シニア全ペルソナが複数シナリオで遭遇（Blog/FlashSale で UNIQUE 違反検知に使いたかった）
- 詳細: Trial 05-C シニアが「v1.5.23 以降のため未収録」と明記。現在は v1.5.22 が最新
- 対応: v1.5.23 として `DatabaseConstraintException` を公開 API に昇格させてリリース
