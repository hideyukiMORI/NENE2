...
## 🚧 現在のレーン — インストーラ toolkit（v1.6.0）

| 項目 | 値 |
|------|-----|
| 現在の VERSION | **1.6.0**（`src/FrameworkInfo.php` が正） |
| 主軸 | 共有ホスティング向け **opt-in インストーラ toolkit `Nene2\Install`** |
| v1.6.0 の成果 | payload セキュリティ核 / preflight & config / TenantConfigurationValidator / DatabaseSchemaApplier / release manifest / ReleaseSource / 提示契約 / 無ブランド参照レンダラ（`src/Install/` 29 ファイル） |
...
| Issue | 内容 |
|-------|------|
| **#1482** | `InstallerStep` に入力型（password 等）を追加し参照レンダラに反映 |
| **#1421** | machine preflight verdict の fingerprint + 署名トークン（#1419 分割 3/3） |

> 設計方針: すべて opt-in・generic。wire しなければ dormant、製品固有の前提（UI/ブランド/語彙/パス）を core に焼かない。詳細は CHANGELOG `[1.6.0]`。
...
**現在のレーン（installer toolkit）:**

- **#1482** — `InstallerStep` に入力型（password 等）を追加し参照レンダラに反映する。
- **#1421** — machine preflight verdict の fingerprint + 署名トークン（#1419 分割 3/3）。
- `CLAUDE.md` スリム化（board 07-05 タスク・763→150-200行目安）。
