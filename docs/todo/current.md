...
- PR #905 (FT180 sortlog v1.5.115) — GitHub Actions 回復待ち
- PR #907 (FT181 reminderlog v1.5.116) — GitHub Actions 回復待ち
- Current branch: `feat/908-ft182-batchlog` — PR 作成予定 (v1.5.117)
...
| FT178 | JSON Merge Patch & ETag | patchlog | — | v1.5.113 | json-merge-patch.md（RFC 7396 null セマンティクス・不変フィールド保護・If-Match 412・V.php 実戦） |
| FT179 | テナント分離 & IDOR 防止 | isolationlog | — | v1.5.114 | tenant-isolation.md（SQL tenant_id スコープ・ヘッダー認証・ボディインジェクション防止・404 vs 403）**クラッカー攻撃試験: ATK-01〜12 全Pass** |
| FT180 | SQL ORDER BY インジェクション | sortlog | — | v1.5.115 (PR #905) | sql-orderby-injection.md（allowlist + in_array strict・PSR-7 二重デコード禁止・null バイト）**VULN-A〜L + ATK-01〜12 全Pass（W型特別 FT）** |
| FT181 | ISO 8601 Datetime バリデーション | reminderlog | — | v1.5.116 (PR #907) | iso-datetime-validation.md・V.php 2バグ修正（+25:00 受け入れ・クロスTZ文字列比較） |
| FT182 | バッチ書き込み API & 部分成功 | batchlog | 36/36 | v1.5.117 (本PR) | batch-api-partial-success.md（V::bodyInt() JSON型混同防止・array_is_list()・MAX_BATCH DoS防止） |
