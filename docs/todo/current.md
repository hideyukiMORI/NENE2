...
| Issue | 内容 |
|-------|------|
| **#1526** | ローカル限定: compose の `DB_NAME` env が `ConfigLoaderTest` の partial-defaults テストを落とす（低優先） |
| **#1527** | ローカル限定: app イメージに ext-zip が無く `PayloadInstallerTest` 8件が Error（低優先） |

※ #1421（machine preflight verdict の署名トークン）は 2026-07-06 完了・クローズ済み。#1514（準拠リンタ横展開ブロッカー）は v1.8.2 で修正済み・2026-07-09 クローズ。
...
- ローカル Docker のフル `composer test` には**既存の環境起因失敗が2種**ある（本体は健全・CI は緑・#1526/#1527 に起票済み）: ① `ConfigLoaderTest::testPartialConstructorDefaultsLayerOverCanonicalDefaults` が compose の `DB_NAME=nene2` env で赤（`docker compose run --rm -e DB_NAME= ...` で回避可） ② `PayloadInstallerTest` 8件が app イメージの ext-zip 欠如で Error。
...
- #1526 / #1527 — ローカル `composer test` を完全緑にする環境整備（低優先）。
