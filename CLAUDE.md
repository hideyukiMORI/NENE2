...
複数の NeNe 系プロジェクトを並行開発するため、ホストポートを固定割り当てする。
...
- 新規プロジェクトは `82xx` / `83xx` / `180xx` レンジ外から空きポートを選ぶ。
- `compose.yaml` のポートを変更する際はこの表も必ず更新する。
...
## 20. フィールドトライアルプロジェクトでの NENE2 参照先
