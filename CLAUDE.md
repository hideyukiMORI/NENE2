...
| 現在のタスク・引き継ぎ | private `nene-origin/internal-docs/nene2/todo/current.md`（運用ログは private へ移設・下記注記参照） |
...
| **FT ループ / ATK・VULN / DX Trial 機構** | private `nene-origin/internal-docs/nene2/todo/ft-loop-workflow.md` |

> **運用ログ（`docs/todo`・`docs/daily`・field-trials・ft-registry 相当）は private `nene-origin/internal-docs/nene2/` に移設済み**（P3・2026-07-18）。最新の作業状況・申し送り・引き継ぎはそちらを読むこと。公開リポの docs は Diátaxis（tutorial/howto/reference/explanation）＋ADR/CHANGELOG のみを正とする。
> 現在の作業レーンと未処理 Issue は必ず private `nene-origin/internal-docs/nene2/todo/current.md` を先に読むこと。
